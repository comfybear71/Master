...
| 2026-03-29 | OAuth URL auto-copy for iPad terminal: When Claude Code shows OAuth login URL, a background watcher on the droplet captures it from `script` log output (Python strips ANSI + line wraps), POSTs to MasterHQ API (MongoDB), terminal page polls and auto-populates the URL bar. User taps Go to open login. One-time setup: `curl -sL https://masterhq.dev/api/terminal/setup \| bash`. APIs: `/api/terminal/oauth-url` (GET/POST/DELETE), `/api/terminal/setup` (serves install script), `/api/terminal/get-url` (server-side WebSocket reader). Fixed: JS template escaping (use `[.]` not `\.`), ANSI stripping (Python `re.sub` with `chr(27)`), PTY line-wrap `\r\n` removal, JSON encoding (Python `json.dumps`), polling continues when URL updates. | Claude Code |
