...
| 2026-05-14 | **MASTER RULES v9 + PROJECT CONSOLE.** Integrated learnings from aiglitch-api strangler migration. Added Rules 9-12 (slice-by-slice migrations, strangler config, sandbox boundary, cross-repo handoffs). Amended Rules 3/4/5. Known Gotchas appendix. PR workflow template (`docs/pr-workflow-template.md`). **Project console rebuilt:** tabbed UI (Overview/Dependencies/Releases/Blueprint) with all 8 repos, authenticated GitHub API, stats bar, structured Blueprint from CLAUDE.md/HANDOFF.md. | Claude Code |
