...
Complete circuit breaker prompt with all variants is at \`docs/prompts/circuit-breaker-prompt.md\`.`,
  ...
  {
    id: "discuss-first-rule",
    title: "Discuss-First Rule + Complete PR Handoff",
    category: "prompts",
    icon: "\u{1F91D}",
    content: `## Discuss-First Rule + Complete PR Handoff

**The most comprehensive single prompt in the collection.** Combines "discuss before coding" with "every PR handoff must include all 5 copy-paste sections including the release tag."

### Why it exists

Two problems this solves:

1. **Claude jumps into coding without confirming the goal.** Too many sessions end with "you built the wrong thing." A 2-minute discussion beats 30 minutes of wrong-direction work.

2. **Inconsistent handoffs waste time on iPad/phone.** Unstructured output means manual rewrites. If every section is a copy-paste code block, one tap = done.
...
**3. PR Description** — inside a markdown code block with Summary,
Changes, Test plan
...
Then the tag description inside a code block with sections for New,
Fixed, Changed.

### Rules about release tags

- **Every PR gets a tag. No exceptions.** Small or large change.
- Check existing tags first — use GitHub Releases or git tag --list
- Tag naming:
  - Small fix: bump patch → v1.2.3-YYYY-MM-DD
  - New feature: bump minor → v1.3.0-YYYY-MM-DD
  - Breaking change: bump major → v2.0.0-feature-name
  - Docs-only: v1.2.3-docs-YYYY-MM-DD
  - Crash recovery: v1.2.3-recovery-YYYY-MM-DD
- Never create the tag yourself — only SUGGEST it.

### Why this format matters

I drive merge + release via GitHub web UI on my phone/iPad. Zero
rewriting. Tap once, paste, done.
...
| Scenario | Action |
|---|---|
| Starting any new Claude session | Paste at session start |
| Claude starts coding without asking | Paste to reset behaviour |
| Handoff missing release tag or has inline prose | Paste to enforce format |
| Working with a new Claude on a project | Paste first thing |

### Relationship to other prompts

| Prompt | Use when |
|---|---|
| **New Session Starter** | Full preservation protocol on MasterHQ |
| **Resume After Crash** | Recovering from a crashed session |
| **PR Handoff Format** (standalone) | Just the handoff, for other projects |
| **Circuit Breaker** | Interrupting a fix spiral |
| **Discuss-First Rule** (this one) | The comprehensive single prompt to rule them all |

**If you only paste one prompt at session start, this is the one.**
