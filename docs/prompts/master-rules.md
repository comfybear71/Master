...
When migrating from a legacy backend to a new one, prefer using the new-backend project itself as the strangler: add a `fallback` rewrite in the new repo's `next.config.ts` that forwards unmatched `/api/*` paths to the legacy origin. Point the public subdomain at the new project.

As endpoints migrate, they add a local route and automatically stop falling through. Zero proxy-config maintenance per endpoint.
...
- When a script fails with 403 on every URL, that's the sandbox — not a real bug.

---

## RULE 12 — Cross-repo handoffs (added 2026-04-18)

When work on one repo requires coordinated changes in another repo that Claude doesn't have access to, deliver a **self-contained brief** the user can paste into the sibling Claude session. The brief MUST include:
...
Do not assume the sibling session has context from this one.

---

## Known Gotchas (appendix)

Not rules — just facts that save 30 minutes of debugging:
